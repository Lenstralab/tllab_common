...
def connect_nuclei_with_cells(nuclei: ArrayLike, cells: ArrayLike) -> np.ndarray:
    ...
    jaccard = cdist(
        np.vstack([nuclei_flat == i for i in i_nuclei]).astype(int) if len(i_nuclei) else np.zeros((0, 0)),
        np.vstack([cells_flat == i for i in i_cells]).astype(int) if len(i_cells) else np.zeros((0, 0)),
        "jaccard",
    )
